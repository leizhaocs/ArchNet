...
LayerFull::LayerFull(Params *params, Layer *prev_layer)
{
    ...
    weights_ = new Tensor<DataType>(sample_size_, 1, 1, prev_layer->sample_size_);
    biases_ = new Tensor<DataType>(1, 1, 1, sample_size_);

    grad_weights_ = new Tensor<DataType>(sample_size_, 1, 1, prev_layer->sample_size_);
    grad_biases_ = new Tensor<DataType>(1, 1, 1, sample_size_);
}
...
void LayerFull::cpu_forward(int realBatchSize, bool train)
{
    ...
    DataType *a = prev_layer_->forwardTensor_->getCpuPtr();
    DataType *b = weights_->getCpuPtr();
    DataType *c = forwardTensor_->getCpuPtr();
...
void LayerFull::cpu_backward(int realBatchSize)
{
    ...
    DataType *a = backwardTensor_->getCpuPtr();
    DataType *b = weights_->getCpuPtr();
    DataType *c = prev_layer_->backwardTensor_->getCpuPtr();
...
void LayerFull::gpu_forward(int realBatchSize, bool train)
{
    ...
    DataType *a = prev_layer_->forwardTensor_->getGpuPtr();
    DataType *b = weights_->getGpuPtr();
    DataType *c = forwardTensor_->getGpuPtr();
...
void LayerFull::gpu_backward(int realBatchSize)
{
    ...
    DataType *a = backwardTensor_->getGpuPtr();
    DataType *b = weights_->getGpuPtr();
    DataType *c = prev_layer_->backwardTensor_->getGpuPtr();
...
{
    float_to_datatype(weights_->size(), weights_->getCpuPtr(), weights+offset);
...
void LayerFull::getWeights(float *weights, int &offset)
{
    ...
    datatype_to_float(weights_->size(), weights+offset, weights_->getCpuPtr());
    offset += weights_->size();
...
std::vector<int> LayerFull::getNumWeights()
{
