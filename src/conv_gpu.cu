...
__global__ void im2col_gpu_kernel(const int n, const DataType *data_im, const int input_h, const int input_w,
        const int filter_h, const int filter_w, const int padding_h, const int padding_w, const int stride_h, const int stride_w,
        const int output_h, const int output_w, DataType *data_col)
{
...
__global__ void col2im_gpu_kernel(const int n, const DataType* data_col, const int input_h, const int input_w,
        const int filter_h, const int filter_w, const int padding_h, const int padding_w, const int stride_h, const int stride_w,
        const int output_h, const int output_w, DataType *data_im)
{
    int index = blockIdx.x*blockDim.x+threadIdx.x;
    for (; index < n; index += blockDim.x*gridDim.x)
    {
